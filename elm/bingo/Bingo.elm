...
update action model =
    case action of
        ...
        Sort ->
            let
                sortedEntries =
                    Debug.log "Sorted Entries: " (List.sortBy .points model.entries)
            in
                { model | entries <- sortedEntries }
        ...
        Delete id ->
            let
                ...
                remainingEntries =
                    Debug.log "Remaining Entries: " (List.filter (hasId id) model.entries)
            in
        ...
        Mark id ->
            let
                ...
                --markedEntry =
                --    { entryToMark | wasSpoken <- True }
                --remainingEntries =
                --    List.filter (\e -> e.id /= id) model.entries
                --markedEntries =
                --    List.concat remainingEntries markedEntry
            in
                { model | entries <- List.map updateEntry model.entries }
                --{ model | entries <- markedEntries}
